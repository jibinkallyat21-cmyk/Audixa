    ...
    <aside className="flex h-screen w-[220px] shrink-0 flex-col overflow-y-auto bg-navy text-white">
      <div className="flex items-center gap-2 px-5 py-6">
        ...
        <div className="leading-tight">
          ...
          <p className="text-[10px] text-white/50">Management</p>
        </div>
      </div>
      ...
      <div className="border-t border-white/10 px-4 py-4">
        <p className="truncate text-sm font-medium text-white">{mgmtUser.name}</p>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full z-[90] mt-3 w-[380px] overflow-hidden rounded-xl border border-slate-200 bg-white text-left shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <h3 className="text-sm font-bold text-navy">Partner-Level Alerts</h3>
              <button
                onClick={() => {
                  setOpen(false)
                  navigate('/management/log')
                }}
